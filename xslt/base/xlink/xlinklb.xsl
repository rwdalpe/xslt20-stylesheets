...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:variable name="lb"
    select="'http://www.w3.org/1999/xlink/properties/linkbase'" />

  <xsl:template
    match="db:info/db:extendedlink[db:link[@xlink:type='arc'
                                                     and @xlink:arcrole=$lb]]">
    <xsl:apply-templates
      select="db:link[@xlink:type='arc' and @xlink:arcrole=$lb
  ...
  <xsl:template match="db:link" mode="load-linkbase">
    ...
    <xsl:variable name="lfrom"
      select="(../*[@xlink:type='locator' and @xlink:label=$from])[1]" />
    <xsl:variable name="lto"
      select="(../*[@xlink:type='locator' and @xlink:label=$to])[1]" />

    <xsl:if test="$lfrom and $lto and $lfrom/@xlink:href=''">
      <xsl:sequence
        select="doc(resolve-uri($lto/@xlink:href, base-uri($lto)))/*/*[@xlink:type='extended']" />
    </xsl:if>
  </xsl:template>
