...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  xmlns:doc="http://nwalsh.com/xsl/documentation/1.0" xmlns:ext="http://docbook.org/extensions/xslt20"
  xmlns:f="http://docbook.org/xslt/ns/extension" xmlns:fn="http://www.w3.org/2005/xpath-functions"
  xmlns:fp="http://docbook.org/xslt/ns/extension" xmlns:ghost="http://docbook.org/ns/docbook/ephemeral"
  xmlns:h="http://www.w3.org/1999/xhtml" xmlns:m="http://docbook.org/xslt/ns/mode"
  xmlns:mp="http://docbook.org/xslt/ns/mode/private" xmlns:t="http://docbook.org/xslt/ns/template"
  xmlns:tp="http://docbook.org/xslt/ns/template/private" xmlns:xs="http://www.w3.org/2001/XMLSchema"
  ...
  <xsl:function name="f:find-node-in-sequence" as="xs:integer">
    ...
    <xsl:choose>
      <xsl:when test="$start &gt; count($nodes)">
        0
      </xsl:when>
      ...
      <xsl:otherwise>
        <xsl:value-of
          select="f:find-node-in-sequence($nodes, $target,
			                            $start+1)" />
      </xsl:otherwise>
    </xsl:choose>
  </xsl:function>
      ...
      <xsl:when test="$iconum &lt;= number($callout.unicode.number.limit)">
        <xsl:choose>
          <xsl:when test="$callout.unicode.start.character = 10102">
            <xsl:choose>
              <xsl:when test="$iconum = 1">
                &#10102;
              </xsl:when>
              <xsl:when test="$iconum = 2">
                &#10103;
              </xsl:when>
              <xsl:when test="$iconum = 3">
                &#10104;
              </xsl:when>
              <xsl:when test="$iconum = 4">
                &#10105;
              </xsl:when>
              <xsl:when test="$iconum = 5">
                &#10106;
              </xsl:when>
              <xsl:when test="$iconum = 6">
                &#10107;
              </xsl:when>
              <xsl:when test="$iconum = 7">
                &#10108;
              </xsl:when>
              <xsl:when test="$iconum = 8">
                &#10109;
              </xsl:when>
              <xsl:when test="$iconum = 9">
                &#10110;
              </xsl:when>
  ...
  <xsl:template name="t:verbatim-patch">
    ...
    <!-- put the lines into a tree so that we can use preceding:: to look for ID 
      values -->
    ...
    <xsl:call-template name="t:callout-bug">
      <xsl:with-param name="conum">
        <xsl:choose>
          ...
          <xsl:otherwise>
            <xsl:number count="ghost:co" level="any"
  ...
  <xsl:function name="f:lastLineNumber" as="xs:decimal">
    ...
    <xsl:variable name="expanded-text" as="node()*">
      <xsl:for-each select="$listing/node()">
        <xsl:choose>
          <xsl:when
            test="self::db:inlinemediaobject
  ...
  <xsl:template match="ghost:line" mode="mp:pl-restore-table-lines">
    ...
    <div
      class="verbatimline {if ($linenumber mod 2 = 0) then 'verbatimeven' else 'verbatimodd'}">
      <div class="vlcontent">
        <xsl:choose>
          <xsl:when test="ghost:end">
            <xsl:call-template name="t:restore-content">
  ...
  <doc:template name="t:restore-content" xmlns="http://docbook.org/ns/docbook">
    ...
    <refdescription>
      <para>
        This template reverses the flattening process performed by
        the
        <code>m:patch-empty-elements</code>
        mode. It replaces pairs of
        empty elements and their corresponding
        <tag>ghost:end</tag>
        s with
        an element that has content.
      </para>
    </refdescription>
  ...
  <xsl:template name="t:restore-content">
    ...
    <!-- FIXME: I totally do not understand why straight-up node comparisons fail 
      here.
    ...
    <xsl:choose>
      <xsl:when test="$areas[@ghost:line = $linenumber]">
        <xsl:call-template name="tp:addcallouts">
          <xsl:with-param name="areas"
            select="$areas[@ghost:line = $linenumber]" />
  ...
  <xsl:template name="tp:wrap-lines">
    ...
    <xsl:for-each-group select="$wrapnodes"
      group-ending-with="ghost:br">
      <ghost:line>
  ...
  <doc:template name="t:unwrap" xmlns="http://docbook.org/ns/docbook">
    ...
    <refdescription>
      <para>This template takes a sequence of nodes. It traverses that sequence
        making sure that any element with the specified name occurs at the
        top-most
        ...
        that it had been nested within.
      </para>
    </refdescription>
    ...
    <refreturn>
      <para>
        The original content sequence modified so that all elements named
  ...
  <xsl:template name="t:unwrap">
    ...
    <xsl:choose>
      ...
      <xsl:when
        test="$content[1] instance of element()
  ...
  <xsl:template match="text()" as="node()*" mode="mp:pl-no-lb">
    ...
    <xsl:choose>
      ...
      <xsl:when
        test="system-property('xsl:vendor') = 'MarkLogic Corporation'
      ...
      <!-- In later versions of 4.2 (starting I don't know when, but at least in 
        4.2-6)
  ...
  <doc:mode name="m:patch-empty-elements" xmlns="http://docbook.org/ns/docbook">
    ...
    <refdescription>
      <para>
        This mode is used to completely flatten all nesting in a tree.
        Every start tag is replaced with an empty tag that has a
        <tag class="attribute">ghost:id</tag>
        attribute. Every end tag is
        replaced with an empty
        <tag>ghost:end</tag>
        element that has an
        <tag class="attribute">idref</tag>
      ...
      <para>
        ...
        <function role="named-template">t:restore-content</function>
        .
      </para>
    </refdescription>
  </doc:mode>
  ...
  <xsl:template match="db:programlisting|db:screen" mode="m:verbatim">
