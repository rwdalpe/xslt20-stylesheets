<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                ...
                <xsl:when test="$glossary.sort != 0">
                  <xsl:apply-templates select="$external.glossary//db:glossentry"
                    ...
                    <xsl:sort lang="{$language}"
                      select="normalize-space(concat(@sortas, db:glossterm[not(parent::db:glossentry/@sortas) or parent::db:glossentry/@sortas = '']))" />
                  </xsl:apply-templates>
                </xsl:when>
                <xsl:otherwise>
                  <xsl:apply-templates select="$external.glossary//db:glossentry"
  ...
  <doc:template name="n:normalize-generated-title" xmlns="http://docbook.org/ns/docbook">
    <refpurpose>
  ...
  <doc:template name="n:normalize-dbinfo" xmlns="http://docbook.org/ns/docbook">
    ...
    <refdescription>
      <para>
        ...
        <tag>info</tag>
        element, then the rest of its contents are also normalized.
      </para>
    </refdescription>
  ...
  <xsl:template
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:value-of select="unparsed-text(resolve-uri($data/@fileref, base-uri(.)))" />
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>
