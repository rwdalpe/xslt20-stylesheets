...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  version="2.0" xmlns:f="http://docbook.org/xslt/ns/extension" xmlns:fp="http://docbook.org/xslt/ns/extension/private"
  xmlns:mp="http://docbook.org/xslt/ns/mode/private" xmlns:db="http://docbook.org/ns/docbook"
  ...
  <!--
    Handles some aspects of transclusion according to (based on namespaces)
    ...
    <xsl:copy>
      ...
      <xsl:apply-templates mode="mp:transclude">
        <xsl:with-param name="prefix"
          select="if (@ta:linkscope) then '' else $prefix" />   <!-- Prevent adding several prefixes on multi-level inclusions -->
      </xsl:apply-templates>
    </xsl:copy>
  </xsl:template>

  <!-- FIMXE: this stripping is there just to have more compact output -->
  <xsl:template match="db:definitions" mode="mp:transclude"
    priority="1">
  </xsl:template>
  ...
  <xsl:template match="db:ref[@name]" mode="mp:transclude">
      ...
      <xsl:choose>
        <xsl:when test="@definitionfile">
          <xsl:variable name="defs"
            select="f:expand-definitions(doc(@definitionfile))" />
    ...
    <xsl:variable name="linkscope"
      select="if (@linkscope) then @linkscope else 'near'" />
    ...
    <xsl:if
      test="count($content/(*|text()[normalize-space(.) ne ''])) > 1 and $ref-xmlid">
      <xsl:message>
        Error xml:id can't be added to definition without single outermost
        element.
      </xsl:message>
    </xsl:if>
    ...
    <xsl:for-each select="$content/node()">
      <xsl:copy>
        <xsl:variable name="xmlid"
          select="if ($ref-xmlid) then $ref-xmlid else @xml:id" />
        <xsl:copy-of select="@* except @xml:id" />
        <xsl:if test="self::*">
          <xsl:choose>
        ...
        <xsl:apply-templates mode="mp:transclude">
          <xsl:with-param name="idfixup" select="$idfixup"
            tunnel="yes" />
  ...
  <xsl:function name="f:definition-for-name" as="node()*">
    ...
    <xsl:variable name="closest-info-with-defs"
      select="$context/ancestor-or-self::*/db:info[db:definitions][1]" />

    <xsl:choose>
      <xsl:when test="$closest-info-with-defs">
        <xsl:choose>
          <xsl:when
            test="$closest-info-with-defs/db:definitions/db:def[@name = $name]">
            <xsl:sequence
              select="($closest-info-with-defs/db:definitions/db:def[@name = $name])[last()]/node()" />
          </xsl:when>
          <xsl:otherwise>
            <xsl:sequence
              select="f:definition-for-name($name, $closest-info-with-defs/../..)" />
          </xsl:otherwise>
        </xsl:choose>
      </xsl:when>
      <xsl:otherwise>
    ...
    <xsl:variable name="doc">
      <xsl:choose>
        <xsl:when test="@xpointer">
          <xsl:sequence
            select="f:transclude(doc($baseuri)//*[@xml:id = current()/@xpointer])" />
        </xsl:when>
        <xsl:otherwise>
  ...
  <xsl:template match="@linkend | @endterm | @otherterm | @startref"
    mode="mp:adjust-idrefs">
  ...
  <xsl:function name="f:nearest-matching-id" as="xs:string?">
    ...
    <xsl:variable name="targets"
      select="$context//*[@xml:id][f:unprefixed-id(@xml:id, .) eq f:unprefixed-id($idref, $context)]" />
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <!-- You might think we should generate a warning message here. The trouble 
          is, it's possible that a downstream step in the pipeline knows what to do with this. 
          So we just leave it alone and assume someone else will generate the error if it is 
          an error. -->
        <xsl:value-of select="$idref" />
      </xsl:otherwise>
    </xsl:choose>
  ...
  <!-- FIXME: type annotation should be without ?, find why it is called with empty 
    sequence -->
    ...
    <xsl:choose>
      ...
      <xsl:when test="matches($static-base-uri, '^[-a-zA-Z0-9]+:')">
        ...
        <!-- this is complicated by two things, first it's not clear when we get 
          file:///path and when we get file://path; second, on a Windows system if we get file://D:/path 
          we have to remove both slashes -->
        <xsl:choose>
