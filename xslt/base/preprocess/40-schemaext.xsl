...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  xmlns:m="http://docbook.org/xslt/ns/mode" xmlns:mp="http://docbook.org/xslt/ns/mode/private"
  xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="db f mp xs"
  version="2.0">

  <!--
    Handles schema extensions as described in
  ...
  <xsl:template match="*" mode="m:schemaextensions"
    xmlns:r="http://nwalsh.com/xmlns/schema-remap/" xmlns:rng="http://relaxng.org/ns/structure/1.0">

    <xsl:variable name="element-name" select="node-name(.)" />
    <xsl:variable name="known"
      select="for $n in $schema-extensions
				     return
